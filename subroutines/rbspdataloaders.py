...
def download_mageis_l3(probe, date, year):
    """
    Download ECT MagEIS L3 (rel04) CDF file for a given probe and date.
    This file contains electron/proton flux AND ephemeris (L, MLT, etc.)
    ...
    probe : str
        'rbspa' or 'rbspb'
    date : str
        Date in YYYYMMDD format (e.g., '20160120')
    year : str
        Year string (e.g., '2016')

    Returns:
    --------
    str : Local filename of downloaded CDF
    """
    base = f'https://spdf.gsfc.nasa.gov/pub/data/rbsp/{probe}/l3/ect/mageis/sectors/rel04/{year}/'
    
    # Filename pattern: rbspa_rel04_ect-mageis-l3_YYYYMMDD_v*.cdf
    filename_pattern = f'{probe}_rel04_ect-mageis-l3_{date}'
    
    cdf_filename = find_cdf_file(base, filename_pattern)
    
    local_filename = 'mageis_l3.cdf'
    urllib.request.urlretrieve(base + cdf_filename, local_filename)
    print(f"Downloaded: {cdf_filename}")
    
    return local_filename

def download_position_daterange(probe, start_date, end_date):
    """
    Download and concatenate Van Allen Probe Position data using MAGEIS L3 data.
    ...
    while current <= end_date:
        ...
        try:
            # Download files for this day
            download_mageis_l3(probe, date_str, year)
            
            # Extract the position data and time data
            mageis_cdf = cdflib.CDF('mageis_l3.cdf')
            data['L'].append(mageis_cdf.varget('L').reshape(-1))
            data['MLT'].append(mageis_cdf.varget('MLT').reshape(-1))
            data['Epoch'].append(mageis_cdf.varget('Epoch'))
